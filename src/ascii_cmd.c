...
void process_ascii_cmd(unsigned char *str) {
    ...
    if (cmd_len > 3 && str[2] == ' ') {
        ...
        if (CMD_MATCHES("PI")) {
            arg[4] = 0;
#ifdef RBDS
            if (arg[0] == 'K' || arg[0] == 'W' || arg[0] == 'k' || arg[0] == 'w') {
    ...
    if (cmd_len > 4 && str[3] == ' ') {
        ...
        if (CMD_MATCHES("RTP")) {
            ...
            } else if (sscanf((char *)arg, "%31[^,],%hhu,%hhu,%31[^,],%hhu,%hhu", tag_names[0], &tags[1], &tags[2], tag_names[1], &tags[4], &tags[5]) == 6) {
